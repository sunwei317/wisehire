...
# This is your existing GET route for serving the upload page
@app.get("/upload", response_class=HTMLResponse)
async def upload_page(request: Request, username: str = None, db: Session = Depends(get_db)):
    if not username:
        return RedirectResponse("/")
...
async def analyze_resume(
    ...
    try:
        # 为每个文件创建任务记录
        for file_index, file in enumerate(resume_files):
            # 验证文件类型
            ...
            if file_extension not in allowed_extensions:
                raise HTTPException(
                    status_code=400, 
                    detail=f"Unsupported file type for {file.filename}. Please provide: {', '.join(allowed_extensions)} documents"
                )
            
            # 验证文件内容
            file_content = await file.read()
            if len(file_content) == 0:
                raise HTTPException(status_code=400, detail=f"Resume file {file.filename} cannot be empty")
            ...
            try:
                ...
                if file_extension not in ['pdf', 'docx', 'txt', 'doc']:
                    raise HTTPException(status_code=400, detail=f"File {file.filename} must be PDF, DOCX, DOC, or TXT format")
                ...
                if file_extension == "pdf":
                    ...
                    # 对于.doc文件，可能需要特殊处理
                    # 这里假设你有处理.doc文件的函数
                    resume_text = extract_text_from_doc(file_path)  # 你需要实现这个函数
                else:  # txt
                    resume_text = extract_text_from_txt(file_path)

                task_with_resume_text_ids.append(task_id)
                resume_texts.append(resume_text)
            
            except Exception as e:
                # 如果单个文件处理失败，更新该任务状态为失败
                update_data = {
                    "task_id": task_id,
                    "status": "failed",
                    "updated_at": datetime.now()
                ...
                print(f"File processing failed for {file.filename}: {str(e)}")
                # 继续处理其他文件，不中断整个流程
            
        submit_pipeline.delay(task_with_resume_text_ids, user_id, resume_texts, job_description)
